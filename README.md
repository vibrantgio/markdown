...
| `markdown/highlight` | A chroma-backed `Highlighter` for fenced code — `New` for the highlighter alone, `Wear` to put a whole syntax highlighter style on the block, background and all. Importing this package is what pulls chroma into a build; no chroma type reaches its exported API. |
...
```go
st := markdown.FromTokens(p, typo, color.NRGBA{})
highlight.Wear(&st, highlight.DefaultStyle, p)
```

A syntax highlighter style is a background, a body colour and a couple of
dozen accents chosen together, and which of them reads most pronounced — and
by how much — is a set of relations that only holds where the whole set is. So the fenced
...
The fourth of those is what keeps a block a block. A style fitted to a light
page is drawn on a near-white and this page is a near-white too, so a fill alone
can leave the reader guessing where the code begins: where a style's background
...
colour. Where the background stands off on its own, no line is drawn. A style
that names no background at all — four of the embedded styles — is drawn on
the fill an inline chip uses, which is what a fence had before any style was
chosen.

Contrast inside the block is surfaced, not enforced. A style shows as its
author drew it, faint ones included; what the library does is measure —
the package's own test sweep records what every style's colours reach on the
background they were drawn on, and names the faintest.

One style name covers both appearances: which member is worn follows the
...
`DefaultStyle` is `catppuccin-latte`, whose registered counterpart
...
Where somebody has chosen a style for each appearance rather than one for
both, hand over the pair and each appearance wears its own member:

```go
p := highlight.StylesOrDefault(lightName, darkName) // resolved and measured
highlight.WearPair(&st, p, c)                       // c decides which member
```

`DefaultStyles()` is the pair that stands in when nothing was chosen —
`catppuccin-latte` and `catppuccin-mocha`. `StylesOrDefault` keeps a member
...
names := highlight.Styles()               // embedded and loaded, sorted
style := highlight.StyleOrDefault(chosen) // a name that no longer resolves
ok := highlight.StyleSuits(style, dark)   // which appearance it was fitted to
```

`StyleSuits` measures the style's own background rather than reading its
...
Somebody who has picked one style has picked one appearance, and
`CompletePair` finds the other half:

```go
p := highlight.CompletePair(chosen)   // both members, whichever side was picked
cols := highlight.StyleColors(chosen) // the colours it draws code with
```
...
measured: of the styles fitted to the opposite background, the one whose
colours fall nearest, class against class, on the OKLCh hue circle, weighted
by how much colour each one actually carries. `StyleDistance` is that measure
on its own. A style fitted to no background at all is a pair by itself. The
measure is held honest by a test that runs it over the declared pairs with the
declarations hidden and requires it to find them anyway.

`StyleColors` is a style as plain colours, for painting swatches or for
...
Contrast in a fence is surfaced and never enforced, and `StyleContrast` is
where it is surfaced:

```go
a, ok := highlight.StyleContrast(chosen) // colours measured, and how many are faint
...
Those markers are not measured at all. A style fitted to no background has no
...
as soon as a fence wears a syntax highlighter style: the block takes that
style's background
...
- **Highlight colours are the chroma style's, and are never re-fitted.** Runs
  ...
  per appearance, or `highlight.WearPair` where a style was chosen per
  appearance. Neither enforces a contrast floor on somebody else's colours. An unrecognised style
