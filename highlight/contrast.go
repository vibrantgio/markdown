// contrast.go — how strongly a style draws, measured on its own background.
//
...
// [StyleContrast] reports against, so that "this style is drawn faint" comes out
...
// AuthoredContrast is what a style's own colours measure against the background
...
type AuthoredContrast struct {
	...
	// each reading class the style colours differently from it.
	Colors int
	// Below is how many of those measure under [ContrastFloor].
	Below int
}

// BelowFloor reports whether this style reads faint: whether most of what it
...
// to nothing, and so does a style that colours no reading run at all: a style
...
func StyleContrast(name string) (AuthoredContrast, bool) {
	s, ok := lookup(name)
	if !ok {
