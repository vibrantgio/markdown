// colors.go — a style read as the colours it holds rather than as a rule for
// drawing code.
//
// A syntax highlighter style is a curated set of colours that somebody spent time on: a couple of
...
// speaks chroma and nothing outside this package is allowed to. So the style
...
// StyleColors returns the colours the named style draws code with, or nil for a
...
//
// The list is the style's colours and not its whole entry table. An entry with
...
func StyleColors(name string) []stdcolor.NRGBA {
	s, ok := lookup(name)
	if !ok {
		return nil
	}
...
// colorsOf is [StyleColors] on a style already resolved.
func colorsOf(s *chroma.Style) []stdcolor.NRGBA {
